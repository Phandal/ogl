 ...
 * side of the screen in the vertex shader using this offset value. solution:
...
int main(void) {
  ...
  shader_t vertexShader =
      shader_load(GL_VERTEX_SHADER, "shaders/vertex_hoffset.glsl");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    GLuint hOffsetUniform = glGetUniformLocation(program, "hOffset");
    glUniform1f(hOffsetUniform, 0.5);
